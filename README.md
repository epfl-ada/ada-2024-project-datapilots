...
In this part, we examine how different beer styles are reviewed across different countries. We approach the features of our analysis in two scopes: first, we start with the number of ratings for different beer styles as features of a given country to determine if certain countries cluster together in terms of beer style preferences. Secondly, we add average ratings for each beer style as additional features. We then group countries into geographic regions and apply principal component analysis to reduce the data to 2 dimensions. We then apply K-means to the 2D data to identify clusters, using an optimal number of clusters chosen based on Silhouette and Inertia scores(Elbow Method). We visualize the resulting clusters and analyze the top 5 beer styles in terms of number of reviews for each cluster.While the United States appeared somewhat unique in its preferences, we generally did not find well-defined, separate clusters of countries in terms of beer preferences, but this might be due to the distribution of reviewer locations in the dataset.

We continue by using the time information in reviews to determine if regional beer style preferences identified in the previous part remain stable over time — supporting the hypothesis that they are influenced by culture — or if they fluctuate, suggesting other contributing factors. Our analysis reveals that, on average, regional beer style preferences show little change over time. However, there are notable cases where tastes evolve for specific beer styles in certain regions, likely influenced by global beer trends. A clear example is the comparison of American IPA and Pale Lager ratings in the North American region. In this region, the time evolution of average ratings shows that American IPA ratings remain relatively stable, while Pale Lager ratings appear to increase over time. This highlights how some beer styles can experience shifts in perception over time within specific regions.
...
**Beer origin bias:** Athénaïs and Melis
